...
# Dont forget to remove connections when leaving.
...
filePort = 50001
fileSocket.bind((server, filePort))
...
def recvFile(message):
    ...
    data, addr = fileSocket.recvfrom(2048)
    try:
        while data:
            file.write(data)
            fileSocket.settimeout(2)
            data, addr = fileSocket.recvfrom(2048)
    except timeout:
        file.close()
    ...
    while True:
        try:
            ...
            if message[0:19] == "%SHOW_SERVER_FILES%":
                ...
                thread = threading.Thread(target=recvFile, args=(message,))
                thread.start()
                done = client.recv(2048)
                if done == b"%IMAGE_COMPLETED%":
...
# Add that when trying to send to user which not exists notify the user.
